...
class filtro_premium_cabello(unittest.TestCase):
    ...
    def test_filtro(self):
        ...
        popup.click()
        CTA_largo = self.driver.find_element(By.XPATH, value='/html/body/div[3]/div/div/nav/div/div/ul/li[4]/a')
        CTA_largo.click()
        titulo = self.driver.find_element(By.XPATH, value='/html/body/div[1]/div/div/div[2]/h1')
        if titulo.text == "LARGO A DOMICILIO":
            actualizar_state("GLITZI", status_test="passed", case_id=50, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=50, run_id=active_id)
        self.assertEqual(titulo.text, "LARGO A DOMICILIO")
