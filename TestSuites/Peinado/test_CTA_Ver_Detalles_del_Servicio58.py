...
class ver_detalles_cabello(unittest.TestCase):
    @classmethod
    def setUpClass(inst):
    ...
    def test_detalles(self):
        ...
        popup.click()
        CTA_detalle = self.driver.find_element(By.XPATH, value='//*[@id="service-40"]/div/div[3]/a')
        CTA_detalle.click()
        description = self.driver.find_element(By.XPATH,
                                               value='//*[@id="service-selected"]/div[1]/div[1]/div[2]/div[1]/div/div/b')
        if description.text == "DESCRIPCIÓN":
            actualizar_state("GLITZI", status_test="passed", case_id=58, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=58, run_id=active_id)
