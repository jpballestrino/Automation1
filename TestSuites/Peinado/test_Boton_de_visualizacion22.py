...
class boton_visualizacion_cabello(unittest.TestCase):
    @classmethod
    def setUpClass(inst):
    ...
    def test_boton(self):
        ...
        if CTA_grid.get_attribute("class") == "btn btn-lg btn-just-icon ml-2 btn-info":
            actualizar_state("GLITZI", status_test="passed", case_id=22, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=22, run_id=active_id)
