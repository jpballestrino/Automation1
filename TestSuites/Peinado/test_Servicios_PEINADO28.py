...
class Servicios_peinado(unittest.TestCase):
    @classmethod
    def setUpClass(inst):
    ...
    def test_Servicios(self):
        ...
        WebDriverWait(self.driver, timeout=3).until(
            lambda d: d.find_element(by=By.CLASS_NAME, value='modal-closes.publicity-button'))
        popup = self.driver.find_element(By.XPATH, value='//button[@style="background: transparent"]')
        popup.click()
        self.cta_ver_servicios = self.driver.find_element(by=By.CLASS_NAME,
                                                          value='btn.btn-warning.text-white.text-uppercase.btn-block')
    ...
    def test_Servicios_cabello(self):
        active_id = get_id_of_active_run("GLITZI")
        cta_peinado= self.driver.find_element(By.XPATH, value='/html/body/div[3]/div/div[2]/div/div/div/div/div[5]/a/div/div[2]/div')
        cta_peinado.click()
        url = self.driver.current_url
        if url == data.Web+"servicios/peinado":
            actualizar_state("GLITZI", status_test="passed", case_id=28, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=28, run_id=active_id)
        self.assertEqual(url, data.Web+"servicios/peinado")
        time.sleep(1)
