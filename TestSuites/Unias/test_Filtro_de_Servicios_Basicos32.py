...
class filtro_basico_uñas(unittest.TestCase):
    @classmethod
    def setUpClass(inst):
    ...
    def test_filtro(self):
        ...
        if titulo.text == "SERVICIOS BÁSICOS A DOMICILIO":
            actualizar_state("GLITZI", status_test="passed", case_id=32, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=32, run_id=active_id)
