...
class Pagar_cita_tarjeta(unittest.TestCase):
    ...
    def test_login_mas_serv_mas_prog(self):
        ...
        CTA_Paypal = self.driver.find_element(By.XPATH,
                                              value='//*[@id="tab5"]/div/div/div/div/div[2]/div/div/ul/li[3]/a/i')
        CTA_Paypal.click()
        ...
        time.sleep(3)
        self.search_field = self.driver.find_element(by=By.ID, value='email')
        self.search_field.send_keys(data.paypal)
        CTA_siguiente = self.driver.find_element(By.XPATH,
                                             value='//*[@id="btnNext"]')
        CTA_siguiente.click()
        self.search_field = self.driver.find_element(by=By.ID, value='password')
        self.search_field.send_keys(data.paypal_password)
        CTA_iniciar = self.driver.find_element(By.XPATH,
                                             value='//*[@id="btnLogin"]')
        CTA_iniciar.click()

        CTA_pagar = self.driver.find_element(By.XPATH,
                                               value='//*[@id="confirmButtonTop"]')
        CTA_pagar.click()
        ...
        if verificar.text == "¿Quieres personalizar tu cita?":
            actualizar_state("GLITZI", status_test="passed", case_id=126, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=126, run_id=active_id)
