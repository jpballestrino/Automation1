...
class Servicios_maquillaje(unittest.TestCase):
    @classmethod
    def setUpClass(inst):
    ...
    def test_Servicios_maquillaje(self):
        active_id = get_id_of_active_run("GLITZI")
        cta_maquillaje = self.driver.find_element(By.XPATH, value='/html/body/div[3]/div/div[2]/div/div/div/div/div[4]/a/div/div[2]/div')
        cta_maquillaje.click()
        url = self.driver.current_url
        if url == data.Web+"servicios/maquillaje":
            actualizar_state("GLITZI", status_test="passed", case_id=27, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=27, run_id=active_id)
        self.assertEqual(url, data.Web+"servicios/maquillaje")
        time.sleep(1)
