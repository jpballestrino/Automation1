...
class filtro_mani(unittest.TestCase):
    @classmethod
    def setUpClass(inst):
    ...
    def test_filtro(self):
        ...
        popup.click()
        CTA_mani = self.driver.find_element(By.XPATH, value='/html/body/div[3]/div/div/nav/div/div/ul/li[2]/a')
        CTA_mani.click()
        titulo = self.driver.find_element(By.XPATH, value='/html/body/div[1]/div/div/div[2]/h1')
        if titulo.text == "MANI Y PEDI A DOMICILIO":
            actualizar_state("GLITZI", status_test="passed", case_id=36, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=36, run_id=active_id)
        self.assertEqual(titulo.text, "MANI Y PEDI A DOMICILIO")
