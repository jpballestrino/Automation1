...
class filtro_relajacion(unittest.TestCase):
    @classmethod
    def setUpClass(inst):
    ...
    def test_filtro(self):
        ...
        popup.click()
        CTA_relajacion = self.driver.find_element(By.XPATH, value='/html/body/div[3]/div/div/nav/div/div/ul/li[3]/a')
        CTA_relajacion.click()
        titulo = self.driver.find_element(By.XPATH, value='/html/body/div[1]/div/div/div[2]/h1')
        if titulo.text == "RELAJACIÓN":
            actualizar_state("GLITZI", status_test="passed", case_id=43, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=43, run_id=active_id)
        self.assertEqual(titulo.text, "RELAJACIÓN")
