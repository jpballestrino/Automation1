...
class Servicios_workshop(unittest.TestCase):
    @classmethod
    def setUpClass(inst):
    ...
    def test_Servicios_workshop(self):
        active_id = get_id_of_active_run("GLITZI")
        cta_workshop = self.driver.find_element(By.XPATH, value='/html/body/div[3]/div/div[2]/div/div/div/div/div[7]/a/div/div[2] ')
        cta_workshop.click()
        url = self.driver.current_url
        if url == data.Web+"servicios/workshops":
            actualizar_state("GLITZI", status_test="passed", case_id=30, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=30, run_id=active_id)
        self.assertEqual(url, data.Web+"servicios/workshops")
        time.sleep(1)
