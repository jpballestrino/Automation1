...
class Servicios_cabello(unittest.TestCase):
    @classmethod
    def setUpClass(inst):
    ...
    def test_Servicios_cabello(self):
        active_id = get_id_of_active_run("GLITZI")
        cta_cabello= self.driver.find_element(By.XPATH, value='/html/body/div[3]/div/div[2]/div/div/div/div/div[3]/a/div/div[2]/div')
        cta_cabello.click()
        url = self.driver.current_url
        if url == data.Web+"servicios/cabello":
            actualizar_state("GLITZI", status_test="passed", case_id=26, run_id=active_id)
        else:
            actualizar_state("GLITZI", status_test="failed", case_id=26, run_id=active_id)
        self.assertEqual(url, data.Web+"servicios/cabello")
        time.sleep(1)
